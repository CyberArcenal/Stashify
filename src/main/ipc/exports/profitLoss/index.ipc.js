...
class ProfitLossExportHandler {
  ...
  async _exportCSV(data, params) {
    ...
    csvContent.push("Generated by,Stashify Financial Management System v2.0");
  ...
  async _exportExcel(data, params) {
    try {
      ...
      workbook.creator = "Stashify Financial System";
    ...
    try {
      ...
      const doc = new PDFKit({
        ...
        info: {
          ...
          Author: "Stashify Financial System",
  ...
  _createCoverPage(doc, data) {
    ...
    doc
      ...
      .fillColor(this.CHART_COLORS.light)
      .text(
        "Stashify Financial Management System",
        margin,
        headerPaddingTop + 92,
        {
          width: pageWidth - margin * 2,
  ...
  _createExpenseAnalysis(doc, data = {}) {
    ...
    breakdown.forEach(
      (
        /** @type {{ category: any; amount: any; percentage: any; }} */ item,
    ...
    const totalExpenses =
      breakdown.reduce(
        (/** @type {number} */ s, /** @type {{ amount: any; }} */ it) =>
          s + (Number(it.amount) || 0),
        0,
      ) || 1;
    ...
    breakdown.forEach(
      (
        /** @type {{ amount: any; category: any; }} */ item,
        /** @type {number} */ idx,
      ) => {
    ...
    try {
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        doc
          ...
          .fillColor("#999999")
          .text(
            `Profit & Loss Report | Generated: ${new Date().toLocaleDateString()} | Stashify Financial v2.0 | Report Type: ${data.metadata.report_type} | Confidential`,
            40,
            doc.page.height - 20,
