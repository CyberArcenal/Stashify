...
const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  ...
  const toggleDropdown = (name: string) => {
    if (!isOpen) return; // no dropdowns in collapsed mode
  ...
  // Auto-open dropdown if current path matches a child (only when expanded)
  useEffect(() => {
    if (!isOpen) return;
            ...
            <>
              ...
              <div
                className={`overflow-hidden transition-[max-height] duration-300 ease-in-out ${
                  isOpen && isOpenDropdown ? "max-h-96" : "max-h-0"
                }`}
              >
                <ul
